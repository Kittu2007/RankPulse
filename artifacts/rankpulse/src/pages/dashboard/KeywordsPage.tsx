...
export default function KeywordsPage() {
  ...
  const trending = [{term:'arm workout home',trend:'+41%',hot:true},{term:'social media tips',trend:'+34%',hot:true},{term:'social seo',trend:'+29%',hot:false}];
  const sets = [{name:'Fitness Content Set',count:12,plat:'ig'},{name:'LinkedIn B2B Set',count:8,plat:'li'},{name:'X Growth Set',count:6,plat:'x'}];
            ...
            {sets.map((s, i) => (
              <div key={i} className="flex justify-between items-center py-2 border-b border-b-[#eaeaea] last:border-b-0 cursor-pointer hover:bg-[#f8f8f8]">
                <div>
                  ...
                  <span className={`text-[8px] font-bold uppercase tracking-wider px-[4px] py-[1px] border text-white inline-block mt-1 ${s.plat === 'ig' ? 'bg-[#d946ef] border-[#a21caf]' : s.plat === 'li' ? 'bg-[#3b82f6] border-[#1d4ed8]' : 'bg-[#111] border-[#000]'}`}>{s.plat}</span>
                </div>
                <span className="text-[11px] font-bold text-[#888]">{s.count} keywords</span>
              </div>
            ))}
            <button className="btn btn-outline btn-sm w-full mt-4 justify-center font-bold text-xs">+ Create Set</button>
          </div>
          <div className="p-4">
            <div className="label-sm text-[var(--black)] mb-3">Keyword Gap vs Competitors</div>
            <div className="border-l-4 border-l-[var(--red)] bg-[#fff5f5] p-3 border border-r-[#eaeaea] border-y-[#eaeaea]">
              <div className="label-sm text-[var(--red)] mb-1">OPPORTUNITY FOUND</div>
              <div className="text-xs text-[#555] font-bold">"fitness creator tips" — your top competitor ranks for this. You don't.</div>
            </div>
            <button className="btn btn-red btn-sm w-full mt-3 justify-center font-bold text-xs">View Competitor Analysis →</button>
          </div>
        </div>
      </div>
    </div>
