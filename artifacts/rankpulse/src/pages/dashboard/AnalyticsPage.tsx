  ...
  return (
    <div className="page-transition min-h-screen flex flex-col bg-[var(--bg)]">
      <div className="page-header flex items-center justify-between flex-wrap gap-4">
        <div><div className="page-kicker">Track your growth</div><div className="d4">Analytics & Reporting</div></div>
      ...
      <div className="flex flex-wrap border-b-2 border-b-[var(--black)]">
        ...
        <div className="flex-1 p-[20px_24px] border-r-2 border-r-[var(--black)] flex flex-col justify-center min-w-[200px] bg-[var(--bg)]">
          <div className="text-[48px] leading-none tracking-[-1px] mb-1" style={{ fontFamily: 'var(--font-d)' }}>24K</div>
          <div className="text-[12px] font-bold uppercase tracking-[1px] text-[#888] mb-1">Estimated Reach</div>
        ...
        <div className="flex-1 p-[20px_24px] border-r-2 border-r-[var(--black)] flex flex-col justify-center min-w-[200px] bg-[var(--bg)]">
          ...
          <div className="text-[12px] font-bold uppercase tracking-[1px] text-[#888] mb-1">Active Penalties</div>
          <div className="text-[11px] text-[#f59e0b] font-bold">↓ Fixed 2 this week</div>
        </div>
        <div className="flex-1 p-[20px_24px] flex flex-col justify-center min-w-[200px] bg-[var(--bg)]">
          <div className="text-[48px] leading-none tracking-[-1px] mb-1" style={{ fontFamily: 'var(--font-d)' }}>3.4%</div>
      ...
      {/* Chart */}
      <div className="p-[32px] border-b-2 border-b-[var(--black)] bg-white">
        ...
        <div className="flex items-end gap-2 h-[120px]">
          {bars.map((h, i) => (
            <div key={i} className="flex flex-col items-center gap-1 flex-1">
              ...
              <span className="text-[8px] font-bold text-[#888] uppercase tracking-wide" style={{ fontSize: '7px' }}>{labels[i].split(' ')[1]}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Platform Breakdown */}
      <div className="grid grid-cols-3 border-b-2 border-b-[var(--black)] max-md:grid-cols-1">
        {platformData.map((p, i) => (
          <div key={i} className={`p-[24px] ${i < 2 ? 'border-r-2 border-r-[var(--black)]' : ''} bg-[var(--bg)]`}>
            <div className="text-[10px] font-bold uppercase tracking-[2px] mb-3" style={{ color: p.color }}>{p.plat}</div>
            <div className="text-[64px] font-black leading-none tracking-[-2px] mb-1" style={{ fontFamily: 'var(--font-d)', color: p.color }}>{p.score}</div>
            ...
            <tbody>
              {weeklyReach.map((r, i) => (
                <tr key={i} className="border-b border-[#eaeaea] hover:bg-[#fafafa]">
                  <td className="p-4 text-sm font-bold">{r.week}</td>
                  <td className="p-4 text-sm font-bold">{r.reach}</td>
                  <td className="p-4 text-sm font-bold text-[#16a34a]">{r.eng}</td>
                  <td className="p-4"><span className="text-[20px] font-black" style={{ fontFamily: 'var(--font-d)', color: r.score >= 75 ? 'var(--black)' : 'var(--red)' }}>{r.score}</span></td>
                  <td className="p-4 text-[var(--red)] font-bold text-sm">{r.pen}</td>
                  <td className="p-4 text-xs font-bold text-[#555]">{r.top}</td>
                </tr>
              ))}
            </tbody>
          </table>
