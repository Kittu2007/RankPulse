  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              <tr className="border-b border-[#eaeaea]">
                ...
                <td className="p-3">
                  <Link href="/dashboard/analyzer" className="btn btn-outline btn-sm">View</Link>
                </td>
              </tr>
              <tr className="border-b border-[#eaeaea]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { href: "/dashboard/analyzer", icon: "📊", title: "Content Analyser", desc: "Score your next post" },
          { href: "/dashboard/keywords", icon: "🔑", title: "Keyword Research", desc: "Find ranking keywords" },
          { href: "/dashboard/hashtags", icon: "#️⃣", title: "Hashtag Generator", desc: "Build your hashtag set" },
        ].map((item) => (
          <Link key={item.href} href={item.href} className="card flex items-center gap-4 cursor-pointer hover:shadow-[6px_6px_0_var(--black)] transition-shadow">
            <div className="w-10 h-10 border-2 border-[var(--black)] flex items-center justify-center text-xl bg-[var(--bg)]">{item.icon}</div>
            <div>
