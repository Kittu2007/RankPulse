...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { data: { first_name: firstName, last_name: lastName, primary_platform: platformValue } }
    });
    ...
    <div className="auth-page bg-[var(--bg)] min-h-screen">
      ...
      <div className="auth-right">
        <div className="max-w-[400px] w-full">
          ...
          <div className="d3 mb-6 text-[48px] text-[var(--black)] leading-none" style={{ fontFamily: "var(--font-d)" }}>CREATE<br />ACCOUNT.</div>
          <form onSubmit={handleSignup} className="flex flex-col gap-4">
            {error && <div className="text-[var(--red)] text-sm font-bold mb-2">{error}</div>}
            ...
            <div className="flex flex-col gap-2">
              ...
              <input className="input h-11 px-3 text-sm font-bold" type="password" placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} required />
            </div>
            ...
            <button type="submit" disabled={isLoading} className="btn btn-red w-full justify-center h-12 text-sm mb-3">
