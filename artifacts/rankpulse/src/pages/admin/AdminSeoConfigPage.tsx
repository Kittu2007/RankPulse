export default function AdminSeoConfigPage() {
  const configs = [
    { plat: 'Instagram', color: 'var(--ig)', bg: '#fdf4ff', params: [
    ...
    { plat: 'LinkedIn', color: 'var(--li)', bg: '#eff6ff', params: [
    ...
    { plat: 'X / Twitter', color: '#111', bg: '#f9f9f9', params: [
    ...
    <div className="flex flex-col min-h-screen pb-10">
      <div className="p-[20px_24px] border-b-2 border-b-[var(--black)] bg-[#f5f2e8] flex justify-between items-center flex-wrap gap-4">
        ...
        <div className="flex gap-2 items-center">
          <button className="btn btn-outline btn-sm font-bold text-[10px] uppercase bg-white">Reset to Default</button>
          <button className="btn btn-red btn-sm font-bold text-[10px] uppercase px-5">Save & Deploy</button>
        </div>
      </div>
            ...
            <div className="flex flex-col flex-1">
              {p.params.map((m, idx) => (
                <div key={idx} className="flex justify-between items-center py-3 px-5 border-b border-[#eaeaea]">
                  <span className="text-[12px] font-bold flex-1">{m.n}</span>
                  <div className="flex items-center gap-1.5">
                    <input type="number" className="input h-8 px-2 text-center text-[12px] font-bold border-2 border-[var(--black)] focus:border-[var(--red)] outline-none rounded-none w-[64px]" defaultValue={m.w} />
                    <span className="text-[10px] text-[#888]">%</span>
                  </div>
                </div>
              ))}
              <div className="flex justify-between items-center py-3 px-5 border-t-2 border-[var(--black)] mt-auto bg-[#fafafa]">
                <span className="text-[11px] font-bold uppercase tracking-[1px]">Total Weight</span>
                <span className="text-[11px] font-bold text-[#16a34a]">{p.params.reduce((a, b) => a + b.w, 0)}%</span>
              </div>
            </div>
          </div>
        ))}
